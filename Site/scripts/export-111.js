...
const file = '111-partage-ami-chiffres.html';
const slug = '111-partage-ami-chiffres';
...
async function run() {
  ...
  console.log('Instagram OK'); await p1.close();
  const p2 = await browser.newPage();
  await p2.setViewport({ width: 1080, height: 1920, deviceScaleFactor: 2 });
  await p2.goto(htmlPath, { waitUntil: 'networkidle0' });
  await p2.addStyleTag({ content: `.infographic { height: 1920px !important; } .content { justify-content: flex-start !important; padding-top: 100px !important; } .title-wrapper { flex: 0 !important; margin-top: 80px !important; } .logo-icon { font-size: 6rem !important; } .logo-text { font-size: 3rem !important; } .title { font-size: 9rem !important; } .website { font-size: 2.4rem !important; margin-top: 60px !important; }` });
  await wait(500);
  await (await p2.$('.infographic')).screenshot({ path: path.join(TIKTOK_V_DIR, slug + '-tiktok-v.png') });
  console.log('TikTok V OK'); await p2.close();
  const p3 = await browser.newPage();
  await p3.setViewport({ width: 1080, height: 600, deviceScaleFactor: 2 });
  await p3.goto(htmlPath, { waitUntil: 'networkidle0' });
  await p3.addStyleTag({ content: `.infographic { height: 600px !important; } .content { padding: 15px 50px !important; } .logo-icon { font-size: 2.8rem !important; } .logo-text { font-size: 1.4rem !important; } .title { font-size: 4.5rem !important; } .website { font-size: 1.6rem !important; }` });
  await wait(500);
  await (await p3.$('.infographic')).screenshot({ path: path.join(TIKTOK_H_DIR, slug + '-tiktok-h.png') });
  console.log('TikTok H OK'); await p3.close();
