const puppeteer = require('puppeteer');
const path = require('path');

const BASE = '/Users/emmanuelblezes/Documents/08_Où va l\'argent /Production interne/Réseaux Sociaux ';
const HTML = path.join(BASE, 'Infographies', 'Sources HTML', '218-taux-recuperation-retraite-generations.html');
const OUT = path.join(BASE, 'Infographies', 'Insta & Autres', '218-taux-recuperation-retraite-generations-instagram.png');

(async () => {
  const browser = await puppeteer.launch({ defaultViewport: { width: 1080, height: 1080, deviceScaleFactor: 2 } });
  const page = await browser.newPage();
  await page.goto('file://' + HTML, { waitUntil: 'networkidle0' });
  await new Promise(r => setTimeout(r, 600));
  const el = await page.$('.infographic');
  await el.screenshot({ path: OUT, type: 'png' });
  await browser.close();
  console.log('✓ Export OK :', OUT);
})();
