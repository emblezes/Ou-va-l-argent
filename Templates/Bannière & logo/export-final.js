...
(async () => {
    ...
    await page.setViewport({ width: 2800, height: 8000, deviceScaleFactor: SCALE });
    ...
    await wallpaper.screenshot({
        path: path.join(outputDir, 'fond-ecran-desktop.png'),
        type: 'png',
        omitBackground: false
    });
    console.log('Exported: fond-ecran-desktop.png (5120x2880 retina)');

    // Export fond d'écran téléphone
    const phone = await page.$('.wallpaper-phone');
    await phone.screenshot({
        path: path.join(outputDir, 'fond-ecran-telephone.png'),
        type: 'png',
        omitBackground: false
    });
    console.log('Exported: fond-ecran-telephone.png (2340x5064 retina)');
